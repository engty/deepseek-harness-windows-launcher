...
// The local route resolves catalog IDs but leaves installation to the App.
// Await the native result; merely accepting a request is never success.
async function launcherInstallFetch(path, options) {
  const response = await fetch(path, options)
  const body = await response.json()
  if (body.managedByLauncher !== true || !Array.isArray(body.launcherCommand)) {
    return new Response(JSON.stringify(body), { status: response.status })
  }
  const bridge = window.webkit?.messageHandlers?.launcherPluginStore
  if (!bridge) throw new Error('请在 DeepSeek Harness 启动器内安装插件。')
  const result = await bridge.postMessage({ arguments: body.launcherCommand })
  return new Response(JSON.stringify(result), { status: result.ok ? 200 : 409 })
}
...
function MarketShell({ locale, onClose, activation }) {
  ...
  // The pinned adapter is updated with the launcher, never by this plugin.
  useEffect(() => { setUpdateInfo({ currentVersion: '0.5.0', updateAvailable: false }) }, [])
  ...
  const runInstall = useCallback((message, port) => {
    ...
    setOperationMessage('')
    launcherInstallFetch('/dsh1024/install', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  const runUninstall = useCallback((message, port) => {
    ...
    operationBusyRef.current = true
    setOperationMessage('')
    launcherInstallFetch('/dsh1024/uninstall', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },

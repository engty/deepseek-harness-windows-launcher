...
async function runTrackedPluginCommand(profile, action, target, progress, extraArgs = []) {
    // The macOS host owns installation, compatibility checks and activation.
    // A store request only prepares intent; it must not mutate the live tree.
    return launcherIntent(['plugin', '--profile', profile,
        action === 'uninstall' ? 'remove' : 'add', ...extraArgs, target]);
}
function launcherIntent(args) {
    return { exitCode: null, timedOut: false, stdout: '', stderr: '',
        managedByLauncher: true, launcherCommand: args };
}
/**
...
async function runReportedPluginCommand(profile, pluginId, action, target, progress, extraArgs = [], versions = {}) {
    return runTrackedPluginCommand(profile, action, target, progress, extraArgs);
}
    ...
    const disposers = [
        ...
        webServer.register({
            ...
            handler: async (request, response) => {
                if (!requireTrustedPost(request, response))
                    return;
                sendJson(response, 409, { ok: false, error: '1024 Store 由启动器维护，请通过启动器版本更新。' });
                return;
